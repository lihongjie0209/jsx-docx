...
**在生成任何文档之前，AI Agent 必须首先调用 `get_component_spec` 工具获取完整的组件规范。**

这是强制要求，因为：
1. 规范定义了所有可用组件和正确的 JSX 语法
2. 避免生成语法错误的文档
3. 确保使用正确的属性名称和数据结构

**正确的流程**：
```
1. 调用 get_component_spec → 获取规范
2. 阅读并理解组件语法
3. 调用 generate_docx → 生成文档
```

❌ **错误流程**：直接调用 `generate_docx` 而不先读取规范
✅ **正确流程**：先 `get_component_spec`，再 `generate_docx`
...
### 1. `get_component_spec` - 获取组件规范 ⚠️ **必须首先调用**

**这是强制性的第一步**：在生成任何文档之前，必须调用此工具获取完整的组件规范和语法参考。
...
**⚠️ 前置要求**：
- **必须先调用** `get_component_spec` 获取并阅读完整规范
- 确认理解所有组件的正确语法和属性
- 然后才能调用此工具生成文档

**强制工作流程**：
1. **第一步（必需）**：调用 `get_component_spec` 了解语法
2. **第二步**：调用 `generate_docx` 生成文档
