...
## `<Footnote>`
在文档中添加脚注引用，脚注内容显示在当前页面底部。脚注常用于学术论文、法律文档和技术文档中添加补充说明或参考文献。
...
<Paragraph>
    <Text>根据最新研究</Text>
    <Footnote text="Smith, J. (2024). Research Paper Title. Journal Name, 1(1), 1-10." />
    <Text>，这种方法更有效。</Text>
</Paragraph>
```

### 注意事项
1. **位置**：`<Footnote>` 应放在 `<Paragraph>` 内部，在文本需要标注脚注的位置。
2. **引用编号**：Word 会自动为脚注分配递增的编号（1, 2, 3...）。
3. **页面底部**：脚注内容显示在脚注引用所在页面的底部。
4. **分隔线**：Word 会自动在脚注区域上方添加分隔线。

- 行为：在引用位置插入脚注标记（上标数字），脚注内容在当前页面底部显示。
- 实现状态：已实现。

---

## `<Endnote>`
在文档中添加尾注引用，尾注内容显示在文档末尾。尾注适用于需要集中展示参考文献或补充资料的场景。
...
<Paragraph>
    <Text>如前所述</Text>
    <Endnote text="[1] 完整参考文献信息" />
    <Text>，该理论已被广泛应用。</Text>
</Paragraph>
```

### 注意事项
1. **位置**：`<Endnote>` 应放在 `<Paragraph>` 内部，在文本需要标注尾注的位置。
2. **引用编号**：Word 会自动为尾注分配递增的编号（通常是罗马数字 i, ii, iii...）。
3. **文档末尾**：所有尾注内容集中显示在文档的末尾部分。
4. **与脚注区别**：脚注在当前页面底部，尾注在文档末尾。
